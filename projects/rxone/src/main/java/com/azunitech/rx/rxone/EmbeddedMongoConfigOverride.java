...
/**
 * Overrides the default configuration to enable journaling. Most of the code is copied from {@link
...
public class EmbeddedMongoConfigOverride {
  ...
  /**
   * Overrides the default embedded mongo configuration to enable journaling. Actual magic happens
   * here.
   *
   * @return Mongod configuration which is used to set up the embedded mongo server as well as the
   *     mongo clients ({@link MongoRepository}, {@link MongoTemplate}, etc.)
   */
  @Bean
  @ConditionalOnMissingBean
  public MongodConfig embeddedMongoConfiguration(EmbeddedMongoProperties embeddedProperties)
      throws IOException {
    ImmutableMongodConfig.Builder builder =
        MongodConfig.builder().version(determineVersion(embeddedProperties));
    EmbeddedMongoProperties.Storage storage = embeddedProperties.getStorage();
    if (storage != null) {
      String databaseDir = storage.getDatabaseDir();
      String replSetName = storage.getReplSetName();
      int oplogSize =
          (storage.getOplogSize() != null) ? (int) storage.getOplogSize().toMegabytes() : 0;
      builder.replication(new Storage(databaseDir, replSetName, oplogSize));

      // This line enables the required journaling. This line is missing from actual spring boot's
      // implementation.
  ...
  private IFeatureAwareVersion determineVersion(EmbeddedMongoProperties embeddedProperties) {
    Assert.state(
        embeddedProperties.getVersion() != null,
        "Set the spring.mongodb.embedded.version property or "
    ...
    return Versions.withFeatures(
        createEmbeddedMongoVersion(embeddedProperties),
        embeddedProperties.getFeatures().toArray(new Feature[0]));
  }

  /** Copied as is from {@link EmbeddedMongoAutoConfiguration} */
  private de.flapdoodle.embed.process.distribution.Version.GenericVersion
      createEmbeddedMongoVersion(EmbeddedMongoProperties embeddedProperties) {
  ...
  private InetAddress getHost() throws UnknownHostException {
    if (this.properties.getHost() == null) {
      return InetAddress.getByAddress(
          Network.localhostIsIPv6() ? IP6_LOOPBACK_ADDRESS : IP4_LOOPBACK_ADDRESS);
    }
