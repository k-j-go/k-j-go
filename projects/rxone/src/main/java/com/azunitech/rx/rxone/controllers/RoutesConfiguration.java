...
public class RoutesConfiguration {
    ...
    RouterFunction<?> routes(PersonRepository personRepository) {
        return route(
                GET("/person/{id}"), request -> ok().body(personRepository.findById(request.pathVariable("id")), Person.class))
                ...
                .andRoute(GET("/create"), request -> {
                    IntStream.range(1, 100).forEach( x -> {
                        personRepository.insert( Mono.just(Person.builder().name("tt").age(x).id( String.format("%d", x)).build())).subscribe();
                    });
                    return ok().contentType(MediaType.APPLICATION_JSON).body(Mono.just("created"), String.class);
                })
